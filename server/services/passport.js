...
passport.deserializeUser((id, done, req) => {
    return req.db.select('id')
        .from('users')
        .where('id', '=', id)
        .then((user, err) => {
...
passport.use('local-login',
    new LocalStrategy({
        ...
        req.db.select('email', 'password').from('users')
            .where('email', '=', email)
            .then(data => {
...
passport.use('local-register',
    new LocalStrategy(
        ...
        async (email, password, done, req) => {
            ...
            const user = req.db('users').insert({'email': email, 'password': hashPassword});
            done(null, user);
        }
    )
);